...
from utils.alwayssafe.planners.lp_optimistic import OptimisticLinearProgrammingPlanner
...
class OptCMDPAgent:
    ...
    def from_discrete_env(cls, env, **kwargs) -> 'OptCMDPAgent':
        terminal = terminal = np.zeros(shape=env.nS, dtype=bool)
        max_reward = 1.0
        min_reward = 0.0
        max_cost = 1.0
        min_cost = 0.0
