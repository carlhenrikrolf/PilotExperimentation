...
# keep in mind:
# [ ] could there be impossible states that the environment is not flagging? Maybe make a specific InvalidStateActionPair exception? in the implementation sof the env ofc
# [ ] need to alter all (de)cellularize, (de)tabularize.
# [ ] here, I'm removing shielding momentarily to make the base algorithm work for the new environment first, I comment out all those parts.
# [ ] make a reset function such that inits can be done more in parallel, lower priority though
...
class PeUcrlAgt:
    ...
    def __init__(
        ...
        """Implementation of a cellularised version of UCRL2."""
        ...
        #######
        self.transition_indicator = np.zeros(
            shape=(self.prior_knowledge.n_states, self.prior_knowledge.n_actions),
            dtype=int,
        )
        for s in range(self.prior_knowledge.n_states):
            state = self.prior_knowledge.detabularize(s, space='state')
            for a in range(self.prior_knowledge.n_actions):
                action = self.prior_knowledge.detabularize(a, space='action')
                if prior_knowledge.is_available(state, action):
                    self.transition_indicator[s, a] = 1
        ...
        #     dtype=int,
        # ) # this needs to change to remove impossible state--action pairs
        ...
        if hasattr(prior_knowledge, 'reward_func'):
            ...
            for s in range(self.prior_knowledge.n_states):
                for a in range(self.prior_knowledge.n_actions):
                    if self.transition_indicator[s, a] == 1:
                        for next_s in range(self.prior_knowledge.n_states):
                            if 1 in self.transition_indicator[s, :]:
                                self.reward_func[s, a, next_s] = self.prior_knowledge.reward_func(
                                    ...
                                    next_state=self.prior_knowledge.detabularize(
                                        next_s,
                                        space='state',
                                    ),
                                )
        # the reward has to incorporate the enire transitions
        # if hasattr(prior_knowledge, 'reward_func'):
        #     self.reward_func = np.zeros(
        #         shape=(self.prior_knowledge.n_states, self.prior_knowledge.n_actions),
        #         dtype=float,
        #     ) # expand array
        #     for s in range(self.prior_knowledge.n_states):
        #         for a in range(self.prior_knowledge.n_actions): # extra for-loop
        ...
        #             ) # there will be invalid states that we need to handle somehow
        ...
        for s in range(self.prior_knowledge.n_states):
            S = self.prior_knowledge.detabularize(
                tabular_element=s,
                space=self.prior_knowledge.state_space #1
            )
        ...
        #self.rc = [0.0] * self.prior_knowledge.n_cells # can I give this a new name
        ...
        for s in range(self.prior_knowledge.n_states):
            for a in range(self.prior_knowledge.n_actions):
                ...
                self.p_distances[s, a] = np.sqrt((14 * self.prior_knowledge.n_states * np.log(2 * self.prior_knowledge.n_actions * self.t / self.prior_knowledge.confidence_level))
                                                / (maxN)) # we should multiply this with the transition indicator I think
                # here I think we need to incorporate the reward function, no? Do I always have to rely on reward distances now, and remove pieces where I use the reward function directly?
    ...
    def max_proba(self, p_estimate, sorted_indices, s, a):
        ...
        if min1 == 1:
            ...
            l = 0
            while sum(max_p) > 1: 
                max_p[sorted_indices[l]] = max([0, 1 - sum(max_p) + max_p[sorted_indices[l]]])  # This estimate should take into account the transition indicator as well, it is not enough that it as input takes the uncertainty intervals. Does this mean that I no longer need the Q multiplied by I?
                l += 1
        return max_p
            ...
            for s in range(self.prior_knowledge.n_states):
                ...
                for a in range(self.prior_knowledge.n_actions):
                    ...
                    if hasattr(self.prior_knowledge, 'reward_func'):
                        optimistic_reward = self.reward_func[s, a] # this needs to change
                    else:
                    ...
                    temp[a] *= self.transition_indicator[s, a] # this mult may now be superfluous?
    ...
    def reward_shaping(self, tabular_state, tabular_action):
        ...
        for si in cellular_state:
            if self.side_effects_funcs[si] == {'safe', 'unsafe'}:
                return self.r_distances[tabular_state, tabular_action] # maybe this causes me to have to zero out r_distances as well?
        return 0.0
...
def shield(self, behaviour_policy, target_policy, p_estimate):
    return True
